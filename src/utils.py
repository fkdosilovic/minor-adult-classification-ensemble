...
def read_dataset_and_labels(dataset_path, labels_path, train_dtype):
...
def read_and_prepare_dataset(
    ...
    X, y = read_dataset_and_labels(dataset_path, labels_path)
    X = X.reshape(params[f"{dataset}_size"], -1)

    assert X.shape[0] == params[f"{dataset}_size"]
    assert X.shape[1] == params["features"]

    # Transform the problem to a binary classification problem.
    if transform_labels:
        y = np.uint8(y >= 18)
